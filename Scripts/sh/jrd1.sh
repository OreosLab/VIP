#!/usr/bin/env bash
## Date: 2021-04-08
## 没错，如果你打开脚本来看就会发现我就是用Ctrl+C, Ctrl+V 大法从lkx0301 大佬 和 e大佬那里抄过来的，版权归他们
## 
 
IFS='' read -r -d '' lrr <<"EOF"
/*
只能用一天。需每天寻找直播抓包
1 0-23 * * * jrd1.js
*/
...
echo "$lrr" | sed -e "s/RRAID/$1/g" > /ql/scripts/jrd1.js

echo "红包雨RRA替换成$1完成"

# 删除旧的定时任务
#sed -i '/jrd1.js/d' /jd/config/crontab.list

#echo "删除旧的jrd1定时任务完成"

# 添加今天的定时任务
# echo "1 0-23$(date "+ %-d %-m") * jtask jrd1.js now"  >>  /jd/config/crontab.list

# echo "添加当天的jrd1红包雨定时任务完成"
