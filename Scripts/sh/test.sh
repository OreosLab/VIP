...
# 控制是否执行变量
read -p "是否替换 config.sh Y/N：" Rconfig
Rconfig=${Rconfig:-'Y'}
read -p "是否替换 extra.sh Y/N：" Rextra
Rextra=${Rextra:-'Y'}
read -p "是否替换 code.sh Y/N：" Rcode
Rcode=${Rcode:-'Y'}
read -p "是否替换 task_before.sh Y/N：" Rbefore
Rbefore=${Rbefore:-'Y'}
...
# 下载 config.sh
if [[ ! -a "$config_shell_path" ]] && [[ $Rconfig = 'Y' ]]; then
...
# 下载 extra.sh
if [[ ! -a "$extra_shell_path" ]] && [[ $Rextra = 'Y' ]]; then
...
# 下载 code.sh
if [[ ! -a "$code_shell_path" ]] && [[ $Rcode = 'Y' ]] ; then
...
# 将 bot 添加到定时任务
if [[ "$(grep -c bot /ql/config/crontab.list)" = 0 ]] && [[ $Rbot = 'Y' ]]; then
