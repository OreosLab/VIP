...
if (nameChinese && nameConfig && envName) {
    ...
    console.debug(
        `【风之凌殇】 友情提示：当前脚本为 ${GetCurrentActivityScriptFileName()}，包含在互助脚本列表中，但未设置 ShareCodeConfigName 或 ShareCodeEnvName 环境变量，将不会尝试在nodejs中生成互助码的环境变量。ps: 两个值目前分别为 ${nameConfig} ${envName}`
    );
