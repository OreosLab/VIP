...
def_json_total() {
    if [[ $2 = value ]]; then
        def_envs_tool $1 | perl -pe "{s| ||g}" | jq -r .$2
    else
...
def_json() {
    if [[ $2 = value ]]; then
        def_envs_tool $1 | perl -pe "{s| ||g}" | grep "$3" | jq -r .$2
    else
...
TempBlock_CK() {
    ...
    if [[ $ori_user_sum -eq $user_sum ]]; then
        echo -e "。"
    elif [[ $ori_user_sum -gt $initial_user_sum ]] && [[ $initial_user_sum -gt $user_sum ]]; then
        echo -e "，其中：已通过环境变量禁用了 $((ori_user_sum - initial_user_sum)) 个 JD_COOKIE，已临时禁止了 $((initial_user_sum - user_sum)) 个 JD_COOKIE。"
    elif [[ $ori_user_sum -eq $initial_user_sum ]] && [[ $initial_user_sum -gt $user_sum ]]; then
        echo -e "，其中：已临时禁止了 $((initial_user_sum - user_sum)) 个 JD_COOKIE。"
    elif [[ $ori_user_sum -gt $initial_user_sum ]] && [[ $initial_user_sum -eq $user_sum ]]; then
        echo -e "，其中：已通过环境变量禁用了 $((ori_user_sum - initial_user_sum)) 个 JD_COOKIE。"
    fi
    echo -e ""
}
